...
public class YamlConfig {
    ...
    /**
     * Builder de partículas (Utilizando própria API)
     *
     * @param section Seção da configuração
     * @Return Builder de partículas
     * @since 23/07/2024
     */
    public XParticleBuilder getXParticleBuilder(String section) {
        String particleString = config.getString(section + ".type");
        ...
        // Cria o builder de partículas coloridas
        if (color != null) {
            XParticleBuilder particleBuilder = new XParticleBuilder(particle).count(amount).extra(extra).offset(offset, offset, offset);
            if (colorTo != null) {
                particleBuilder.colorTransition(color, colorTo);
                return particleBuilder;
            }
        ...
        if (materialString != null) {
            ...
            return new XParticleBuilder(particle).count(amount).extra(extra).offset(offset, offset, offset).data(material.createBlockData());
        }
        return new XParticleBuilder(particle).count(amount).extra(extra).offset(offset, offset, offset);
    }
